...
module.exports = async function run() {
    ...
    const templateconfig = await handel.existFolderOrFile('templateconfig.json')
    if (!templateconfig) {
        console.log(
    ...
    const existTemplatepathFolder = await handel.existFolderOrFile(
        `${templatepath}`
    )
    const existComponentPlacementPathFolder = await handel.existFolderOrFile(
    ...
    // 3.
    const template = await handel.getFolderOrFiles(templatepath)
    ...
    return new Promise(async function(resolve, reject) {
        ...
        const existFileFolder = await handel.existFolderOrFile(
